...
- `SUPABASE_URL` / `SUPABASE_SECRET_KEY` — your Supabase project's API URL and secret (service_role) key (Supabase dashboard → Project Settings → API). This is the app's only runtime connection to the database, via `backend/supabase_client.py`.
- `DIRECT_URL` — a raw Postgres connection to the same project (Project Settings → Database → Connection string), used only by Alembic migrations and the test suite's table-truncation fixture. See `.env.example` for the exact placeholder format for both.

Schema is managed by [Alembic](https://alembic.sqlalchemy.org/) - there's nothing to run by hand. The backend applies any pending migration automatically on startup (`backend/db.py`'s `run_migrations()`, called from `lifespan`), including creating every table from scratch the first time it connects to a fresh Supabase project. To change the schema: edit `backend/models.py`, then run `alembic revision --autogenerate -m "..."` and commit the generated file in `alembic/versions/` - the app applies it automatically next time it starts. The running app itself never queries through these SQLAlchemy models - all runtime reads/writes go through the Supabase client (`backend/services/ticket_service.py`); `backend/models.py` exists purely so Alembic has a schema to diff against.
...
- **Backend**: FastAPI, querying Supabase Postgres via the Supabase client SDK; schema managed separately by SQLAlchemy models + Alembic (autogenerated, auto-applied on startup)
