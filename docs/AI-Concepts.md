...
Rather than asking the model to "please return JSON" and hoping, this project uses Groq's OpenAI-compatible function-calling as a JSON-schema enforcement mechanism (see [`ticket_router/ai/groq_provider.py`](../ticket_router/ai/groq_provider.py)). Groq exposes an OpenAI-compatible Chat Completions API (`https://api.groq.com/openai/v1`), so we talk to it with the official `openai` Python SDK, just pointed at a different base URL:
